...
def _answer() -> InvestigationAnswer:
    ...
    return InvestigationAnswer(
        ...
        hypotheses=[
            Hypothesis(
                hypothesis_id="H1", description="Authentication timeout",
                rationale="The failure_reason field and the post-mortem agree.",
                status=HypothesisStatus.SUPPORTED, confidence=0.92, prior=0.25,
                supporting_evidence=["evd_1", "evd_2"], contradicting_evidence=[],
                disproof_probe="A successful auth call for TX82931 within the failure window.",
                disproof_searched=True, verified=True,
                verification_note="Two independent sources; no unresolved contradiction.",
                predicted_signals=["auth_timeout in the transaction record"],
            ),
            Hypothesis(
                hypothesis_id="H2", description="Fraud-rule rejection",
                status=HypothesisStatus.DISPROVED, confidence=0.04,
                contradicting_evidence=["evd_1"],
                disproof_probe="A fraud rule firing for TX82931.", disproof_searched=True,
            ),
        ],
        contradictions=[
            Contradiction(
        ...
        claims=[Claim(claim_id="clm_1", text="Authentication timeout caused the failure",
                      confidence=0.94, evidence_ids=["evd_1", "evd_2"])],
        application_links=[
        ...
        autopsy=SearchAutopsy(
            ...
            hypotheses_generated=4, hypotheses_disproved=1, evidence_diversity=0.95,
        ),
